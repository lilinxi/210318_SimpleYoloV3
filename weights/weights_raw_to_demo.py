...
if __name__ == '__main__':
    # 读取原始权重列表
    raw_weights_file = open('./raw_yolov3_weights_state_dict.txt', 'r')
    raw_weights_lines = raw_weights_file.readlines()
    print("raw_weights_lines:", len(raw_weights_lines))
    # 读取新的权重列表
    demo_weights_file = open('./demo_yolov3_weights_state_dict.txt', 'r')
    ...
    raw_weights: dict = torch.load('./raw_yolov3_weights.pth', map_location=device)
    # 一一映射
    ...
    # 保存新的权重
    torch.save(demo_weights, "./demo_yolov3_weights.pth")
